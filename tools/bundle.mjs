...
const viewerScript = bundle(resolve(root, 'src/viewer.js'));

for (const [page, out] of [['embed.html', 'voxel-qr-embed.html'], ['view.html', 'voxel-qr-view.html']]) {
  const pageHtml = readFileSync(resolve(root, page), 'utf8')
    .replace('<link rel="stylesheet" href="./assets/styles.css">', `<style>\n${css}\n</style>`)
    .replace('<script type="module" src="./src/viewer.js"></script>', `<script type="module">\n${viewerScript}\n</script>`);
  writeFileSync(resolve(root, 'dist', out), pageHtml);
  console.log(`dist/${out} ${(pageHtml.length / 1024).toFixed(1)} kB`);
}
