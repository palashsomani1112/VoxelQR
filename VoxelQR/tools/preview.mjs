...
import { paletteFor, groundFor, mixHex, SCAN_GROUND, scanInk, groundTintFor } from '../src/palettes.js';
...
function scenePolys(scene, colors, ground, flip, ink) {
  ...
  const shadeAmount = 0.3 * Math.max(0, 1 - flip / 0.7);
  if (flip < 0.7) {
...
function themedGround(config) {
  ...
  ground.tint = groundTintFor(paletteFor(config.palette), groundFor(config.ground), config.tint);
  return ground;
}
