...
Content lives on the left, look lives along the bottom, and both are collapsible. They are
`<details>` elements, so keyboard support, screen reader semantics and the open state come from the
browser rather than from hand-rolled ARIA. Collapsing the left drawer narrows it to a rail and the
scene takes the space. Below 1000px both drawers stack under the scene, and the bottom drawer starts
closed so the diorama is the first thing you see on a phone.
...
Modes are `templates`, `palettes` and `flip`.
...
`Copy embed code` and `Copy link` both write to the clipboard and also drop the text into a
...
1. **Layout, at three widths.** Toggle the device toolbar, then check 390px, 768px and 1440px. Watch
   for the left drawer moving below the scene at 1000px and the fields going single column at 560px.
   Collapse and expand both drawers at each width and confirm the canvas resizes rather than
   stretching.
...
4. **Touch and performance.** With touch emulation on, tap the scene to flip it and confirm nothing
   depends on hover. Then record a Performance trace during a flip at 4x CPU throttling and look at
   the frame times, since the flip writes a transform for every instance each frame.

The one thing worth doing before any of that: put a phone camera on the scan view and check it
actually scans.

## A note on the collapsible drawers

They are `<details>` elements, and current Chrome wraps the non-summary children in a
`::details-content` box. That changes which element is the grid item, which broke the content
panel's scrolling the first time round: the body grew instead of scrolling and the collapsed
subsections at the bottom were unreachable. The stylesheet now makes both paths work, styling
`::details-content` as the scroll container where it exists and the body itself where it does not.
Worth knowing before you restyle those rules.
...
Shadows fall toward the viewer while the key light also lights the faces you can see, which is not
physically consistent. It is the version that reads, and flat illustration cheats the same way. The
constant to change is `SHADOW_LIGHT` in `src/render/shadows.js`.
