...
import { paletteFor, groundFor, SCAN_GROUND, mixHex, sceneGround } from '../palettes.js';
...
export function createDiorama(container, { overlay, onState, onView, interactive = true } = {}) {
  ...
  function build(config, code) {
    ...
    stage.setView({ azimuth: config.azimuth, elevation: config.elevation });
    stage.setWeather(weather, { fogColor: backdropColor(config) || '#DCE2DE' });
  ...
  function frame() {
    ...
    stage.setWeather(current.weather, {
      fogColor: backdropColor(current.config) || '#DCE2DE',
      flip: state.flatten,
    });
