...
import { NOTE_LIMIT } from '../src/export/note.js';
...
const allFields = CONTENT_TYPES.flatMap((type) => type.groups.flatMap((group) => group.fields));

const PERSONAL = [
  'palash', 'somani', 'liberty mutual', 'frankfurt', 'hauptwache', 'hesse',
  '60313', '1701234567', '353857832760', 'palashsomani',
];
for (const field of allFields) {
  const text = `${field.placeholder || ''} ${field.hint || ''}`.toLowerCase();
  const found = PERSONAL.filter((term) => text.includes(term));
  check(`${field.key} placeholder carries no personal detail`, found.length === 0, found.join(','));
}
check('placeholders are illustrative, not empty',
  allFields.filter((field) => field.type !== 'checkbox' && field.type !== 'select')
    .every((field) => Boolean(field.placeholder)));

const conditional = allFields.filter((field) => field.when);
...
check('rain is not foggy', WEATHER.rain.fog === 0, `${WEATHER.rain.fog}`);
check('fog is the foggiest',
  Object.entries(WEATHER).every(([id, spec]) => id === 'fog' || spec.fog < WEATHER.fog.fog));
check('rain keeps the scene lit', WEATHER.rain.key >= 0.9 && WEATHER.rain.ambient <= 0.7,
  `${WEATHER.rain.key}/${WEATHER.rain.ambient}`);
check('rain keeps its shadows', WEATHER.rain.shadow >= 0.5, `${WEATHER.rain.shadow}`);
check('rain tints blue rather than grey', (() => {
  const hex = WEATHER.rain.tint.replace('#', '');
...
check('the note field has a label', contentJs.includes('noteLabel.htmlFor = noteId'));
check('the note field has a described hint', contentJs.includes("noteInput.setAttribute('aria-describedby'"));
check('the note field is length capped', contentJs.includes('noteInput.maxLength = NOTE_LIMIT'));
check('the note position control has a label', contentJs.includes('posLabel.htmlFor = posId'));
check('every page can show a note',
  [html, view, embed].every((page) => page.includes('id="note"')));
check('the note starts hidden', /id="note"[^>]*hidden/.test(html));
check('note position is driven by data, not two classes',
  /\.note\[data-position="above"\]/.test(css) && /\.note\[data-position="below"\]/.test(css));
check('note ordering is explicit for every sibling',
  /\.stage-wrap \{[^}]*order: 0/s.test(css) && /\.viewer-foot \{[^}]*order: 2/s.test(css));
check('the note wraps rather than overflowing', /\.note \{[^}]*overflow-wrap: break-word/s.test(css));
check('the note stays readable on any backdrop', /\.note \{[^}]*color: var\(--on-backdrop\)/s.test(css));
check('a note replaces the gesture hint when sharing',
  read('src/viewer.js').includes('hintEl.hidden = true'));
check('the embed page can place a note above the code', embed.includes('id="note"'));
check('note text is capped well under the payload limit', NOTE_LIMIT <= 200, `${NOTE_LIMIT}`);
