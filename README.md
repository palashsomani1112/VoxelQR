...
- **Contact** is grouped into Name, Work, Reach, Address, Social and a contact note, with the last
  three collapsed until you have something in them. Socials take a handle or a full link and are written as
...
An optional note travels with the code. Type it in the Content panel and pick whether it sits above
or below, and it shows up in four places: beside the scene in the app, in the shared link, in the
embed, and baked into the PNG and SVG exports. Leave it empty and nothing changes.

It is capped at 180 characters, which wraps to about four lines at the width of the code. Wrapping is
done by a pure function shared by all four paths, so the SVG, the PNG and the screen break the lines
in the same places.

The note never intrudes on the code itself. It is laid out in a block above or below the square, so
the four module quiet zone stays clear and the code stays scannable. On the shared and embedded
views, a note replaces the gesture hint rather than sitting alongside it, so the only words your
reader sees are yours.
...
Seventeen objects ship with it, all encoding the same link:
